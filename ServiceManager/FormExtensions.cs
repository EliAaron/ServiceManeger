...
namespace ServiceManager
{
    ...
    {
        public static Form Grayscale(this Form tocover)
        {
            var frm = new Form
            {
            ...
            tocover.SizeChanged += (sender, args) =>
            {
                frm.Size = tocover.ClientSize;
                frm.Location = tocover.PointToScreen(tocover.ClientRectangle.Location);
            };

            tocover.Resize += (sender, args) =>
            {
                frm.Size = tocover.ClientSize;
                frm.Location = tocover.PointToScreen(tocover.ClientRectangle.Location);
            };

            tocover.ResizeEnd += (sender, args) =>
            {
                frm.Size = tocover.ClientSize;
                frm.Location = tocover.PointToScreen(tocover.ClientRectangle.Location);
            };

            tocover.LocationChanged += (sender, args) =>
            {
                frm.Size = tocover.ClientSize;
                frm.Location = tocover.PointToScreen(tocover.ClientRectangle.Location);
            };
